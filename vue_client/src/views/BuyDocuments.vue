<template>
    <div class="container">
      <h1>Список закупочных документов</h1>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="buy_doc in buy_docs" :key="buy_doc.id">
            <td>{{ buy_doc.id }}</td>
            <td>{{ buy_doc.buy_num }}</td>
            <td>{{ buy_doc.buy_date }}</td>
            <td>{{ buy_doc.supplier_name }}</td>
            <td>{{ buy_doc.accepted }}</td>
            <td>
              <button @click="showAddClientForm(buy_doc.id)">Открыть</button>
              <button @click="showDelClientForm(buy_doc.id)">Удалить</button>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  export default {
    ...
    mounted() {
      ...
      axios.get('http://127.0.0.1:5000/get_buy_docs')
        .then(response => {
          // Установка полученных данных в свойство clients
          this.buy_docs = response.data;
        })
        .catch(error => {
